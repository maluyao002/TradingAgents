...
from .sources import DiscoveryStatus, PublicSourceService, SourceAccessError
...
class PublicEvidenceCollector:
    ...
    def collect(self, request: ResearchRequest) -> EvidenceSnapshot:
        ...
        if discovered.status == DiscoveryStatus.UNAVAILABLE:
            gaps.append("critical: SEC filing discovery unavailable; this is not evidence of no events.")
        ...
        if discovered.continuation_urls:
            gaps.append("Older SEC index pages remain uncollected; five-year/eight-quarter coverage is unproven.")
        ...
        for filing in discovered.filings[:self.max_filings]:
            ...
            document = SourceDocument(
                id=identifier, url=fetched.final_url,
                title=f"{request.ticker} {filing.form} {filing.filing_date}",
                publisher="SEC", retrieved_at=fetched.retrieved_at,
                published_at=filing.accepted_at, content=fetched.text,
                content_sha256=fetched.text_sha256, accession=filing.accession,
                origin_id=filing.accession, kind="filing",
            )
        ...
        if documents:
            try:
                ...
                if fetched.text is not None and fetched.text_sha256:
                    documents.append(SourceDocument(
                        id="sec:companyfacts", url=fetched.final_url,
                        title="SEC companyfacts acquisition snapshot (not independently dated evidence)",
                        publisher="SEC", retrieved_at=fetched.retrieved_at, content=fetched.text,
                        content_sha256=fetched.text_sha256, kind="other"))
            except (SourceAccessError, ValueError, UnicodeError):
                gaps.append("critical: companyfacts unavailable or invalid; no replacement values invented.")
        if not facts:
            gaps.append("critical: no eligible normalized financial facts.")
        return EvidenceSnapshot(ticker=request.ticker, cutoff=request.cutoff,
                                sources=tuple(documents), facts=facts,
                                gaps=tuple(gaps), instrument=instrument)
