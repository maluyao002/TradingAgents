...
def preview_saved_reader(source: Path, request, destination: Path):
    ...
    if engine not in {f"research-v2-preview-{n}" for n in range(1, 10)}:
        raise ValueError("unsupported preview engine revision")
    requires_provenance = engine in {f"research-v2-preview-{n}" for n in range(6, 10)}
    ...
    if engine in {f"research-v2-preview-{n}" for n in range(7, 10)}:
    ...
    if engine in {f"research-v2-preview-{n}" for n in range(7, 10)}:
        ...
        if case_context.cashflow_bridge is not None:
            if engine != "research-v2-preview-9":
    ...
    if provenance is not None:
        ...
        expected = reader_provenance(authored, draft, eligible_facts, calculations,
                                     ...
                                     bind_case_state=engine in {"research-v2-preview-8", "research-v2-preview-9"})
