...
def preview_saved_reader(source: Path, request, destination: Path):
    ...
    if engine not in {f"research-v2-preview-{n}" for n in range(1, 9)}:
        raise ValueError("unsupported preview engine revision")
    requires_provenance = engine in {"research-v2-preview-6", "research-v2-preview-7", "research-v2-preview-8"}
    ...
    if engine in {"research-v2-preview-7", "research-v2-preview-8"}:
    ...
    if provenance is not None:
        ...
        expected = reader_provenance(authored, draft, eligible_facts, calculations,
                                     ...
                                     bind_case_state=engine == "research-v2-preview-8")
    ...
    preview = render_reader(request, draft, snapshot, issues, request.report_language, compact=True,
                            case_context=case_context, bind_case_state=engine == "research-v2-preview-8")
